...
from aitrade.backtest.t0.tick_policy import (
    DailyBar, DailyHistory, FixedTick, VolScaledTick, TrendTiltTick,
)
...
def test_fixed_tick_returns_constants_asymmetric() -> None:
    s, b = FixedTick(sell_tick=0.02, buy_tick=0.03).ticks_for(date(2024, 2, 1), DailyHistory())
    assert (s, b) == (0.02, 0.03)


def test_vol_scaled_tick_scales_with_range() -> None:
    hist = _hist([0.10] * 20)                       # 近 20 日均振幅 0.10
    s, b = VolScaledTick(k=0.4, n=20, pricetick=0.01).ticks_for(date(2024, 3, 1), hist)
...
def test_vol_scaled_tick_fallback_when_insufficient_history() -> None:
    ...
    s, b = VolScaledTick(k=0.4, n=20, pricetick=0.01, fallback=0.02).ticks_for(date(2024, 3, 1), hist)
    assert s == b == 0.02


def test_trend_tilt_buys_closer_in_uptrend() -> None:
    hist = _hist([0.06] * 10, closes=[10 + i * 0.2 for i in range(10)])   # 上行
    s, b = TrendTiltTick(base=0.02, tilt=0.01, n=5).ticks_for(date(2024, 3, 1), hist)
