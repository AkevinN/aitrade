...
|:---:|------|:---:|:---:|:---:|:---:|------|
| 迭代 0 | 评估诚实性修复 | P0 | 🟢 低 | ✅ 完成 | Claude | 种子真生效 / 信号自描述 objective + 阈值校验 / 对齐丢弃率报警；详见 `.kiro/specs/cnn-eval-honesty-fixes/`，分支 `feat/cnn-eval-honesty-fixes` |
...
| 2026-06-13 | v0.4 | 迭代 0 诚实性修复落地——种子真生效（多种子 WF 聚合）/ 信号自描述 objective + 阈值校验（三处接入）/ 对齐丢弃率报警（训练+推理双路透出）；全链路三项，Property 1~7 Hypothesis 守护，全量 973 passed；进度跟踪表新增「迭代 0」行。 | Claude |
